// Obrazac poslovodja/radnik (master/slave): glavna nit (poslovodja) stvara
...
// poslovodja zbroji sve kvadrate. Centralizirana kontrola: poslovodja je
...
int main(void) {
    ...
    // Poslovodja dodjeljuje posao
    for (int i = 0; i < BROJ_RADNIKA; i++) {
        zadaci[i].ulaz = i + 1;
        pthread_create(&niti[i], NULL, radnik, &zadaci[i]);
    }

    // Poslovodja prikuplja i objedinjuje rezultate
    int zbroj = 0;
    for (int i = 0; i < BROJ_RADNIKA; i++) {
    ...
    printf("Poslovodja: zbroj kvadrata = %d\n", zbroj);
    return 0;
}
